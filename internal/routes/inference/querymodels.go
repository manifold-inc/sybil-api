...
func (im *InferenceManager) QueryModels(c *setup.Context, req *shared.RequestInfo) (*shared.ResponseInfo, *shared.RequestError) {
	/*
		...
		modelMetadata, err := im.DiscoverModels(c.Request().Context(), req.UserID, req.URL)
		if err != nil {
		...
		if err != nil && !canceled {
			...
			metrics.ErrorCount.WithLabelValues(modelMetadata.UID, req.Endpoint, fmt.Sprintf("%d", req.UserID), "request_failed").Inc()
		...
		if res != nil && res.StatusCode != http.StatusOK && !canceled {
			...
			metrics.ErrorCount.WithLabelValues(modelMetadata.UID, req.Endpoint, fmt.Sprintf("%d", req.UserID), "request_failed_from_error_code").Inc()
		...
		scanner:
			...
			if !hasDone && ctx.Err() == nil {
				...
				metrics.ErrorCount.WithLabelValues(modelMetadata.UID, req.Endpoint, fmt.Sprintf("%d", req.UserID), "streaming_no_done")
			}
			if err := reader.Err(); err != nil && !errors.Is(err, context.Canceled) {
				c.Log.Errorw("encountered streaming error", "error", err)
				metrics.ErrorCount.WithLabelValues(modelMetadata.UID, req.Endpoint, fmt.Sprintf("%d", req.UserID), "streaming")
			}
		}
		...
		if !req.Stream && !canceled { // Handle non-streaming response
			...
			if err != nil && ctx.Err() == nil {
				...
				metrics.ErrorCount.WithLabelValues(modelMetadata.UID, req.Endpoint, fmt.Sprintf("%d", req.UserID), "query_model").Inc()
